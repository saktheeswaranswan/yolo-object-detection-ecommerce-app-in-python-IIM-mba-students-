...
while True:
    ...
    for i in indices:
        ...
        pdf.multi_cell(0, 20, txt=search_query, align='L')
        for shop, link in links.items():
            pdf.set_font('Arial', 'U', size=12)
            pdf.cell(0, 10, txt=shop, link=link, ln=1, align='L')

    # Update the PDF every 5 minutes
    current_time = time.time()
    if current_time - start_time >= update_interval:
        # Save the DataFrame to a CSV file
        df.to_csv('detected_links.csv', index=False)

        # Save and close the PDF file
        pdf.output('detected_objects.pdf')
